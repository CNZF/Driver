...
////
    ...
    self.arrModel = @[@[@"头像"],@[@"姓名",@"联系方式",@"驻地",@"车牌号",@"用户类型",@"所属公司",@"修改登录密码"]];
    self.info = USER_INFO;
}
...
- (void)LogoutAction{
    ...
    [[NSUserDefaults standardUserDefaults] setObject:nil forKey:@"GeTuiCID"];
...
//    [self.navigationController popToRootViewControllerAnimated:YES];
    ...
    MLNavigationController *nav=[[MLNavigationController alloc] initWithRootViewController:controller];
    //设置导航条背景图片
    if ([[[UIDevice currentDevice] systemVersion] floatValue] <7.0)
    {
        [nav.navigationBar setBackgroundImage:[UIImage imageNamed:@"bg_black"] forBarMetrics:UIBarMetricsDefault];
    }else{
        [nav.navigationBar setBackgroundImage:[UIImage imageNamed:@"bg_black.png"] forBarMetrics:UIBarMetricsDefault];
    }
    //设置导航条字体颜色
...
//        [self.navigationController pushViewController:vc animated:YES];
    ...
    if (indexPath.row == 2 && indexPath.section == 1) {
        ...
        [vC setBlock:^(CityModel *model) {
//            ws.info.region_name = model.startPosition;
            UserInfoModel * info = USER_INFO;
            info.region_name = model.startPosition;
            [NSKeyedArchiver archiveRootObject:info toFile:[MyFilePlist documentFilePathStr:@"UserInfo.archive"]];
            ws.info = USER_INFO;
            [ws.tvList reloadData];
        }];
        [self.navigationController pushViewController:vC animated:YES] ;
