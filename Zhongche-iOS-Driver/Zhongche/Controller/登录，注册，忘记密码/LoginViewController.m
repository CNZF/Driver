...
//     AppDelegate * app = (AppDelegate *)[UIApplication sharedApplication].delegate;
    ...
    if (userInfo.userType != 3 && userInfo) {
        //                    [ws.navigationController pushViewController:[ZCMyOrderViewController new] animated:NO];
        SelfDefineTabBarViewController * controller = [[UIStoryboard storyboardWithName:@"Business" bundle:nil]instantiateViewControllerWithIdentifier:@"SelfDefineTabBarViewController"];
        MLNavigationController * navi = [[MLNavigationController alloc]initWithRootViewController:controller];
        AppDelegate * app = (AppDelegate *)[UIApplication sharedApplication].delegate;
        app.window.rootViewController = navi;
        
        AdView * adView = [[AdView alloc] init];
        adView.frame = CGRectMake(0, 0, SCREEN_W, SCREEN_H);
        [app.window addSubview:adView];
    }else {
        // identStatus
        //quaStatus
        
        if (userInfo.type == 2 && userInfo.userType == 3) {
...
//    self.loginBtn.enabled = (self.TelField.text.length>0 && self.passwordField.text.length>0);
...
//
//    self.tfPhone.text = self.tfPassword.text = @"";
////     =@"";
//
//    UserInfoModel *userInfo = USER_INFO;
//    if (userInfo) {
...
//    self.loginBtn.enabled = NO;
    ...
    [self setFieldPreference];
//    13341195865
//    13718931400
    AppDelegate * app = (AppDelegate *)[UIApplication sharedApplication].delegate;
    if (app.DeBug) {
//        self.TelField.text = @"13718931400";
//        self.passwordField.text = @"123456";
//        self.TelField.text = @"13501182663";
//        self.passwordField.text = @"zhongche123";
    }
    
//花花
//        self.TelField.text = @"15245623568";
//        self.passwordField.text = @"123456";
...
- (void)setFieldPreference
{
    [self.TelField setValue:[UIColor whiteColor] forKeyPath:@"_placeholderLabel.textColor"];
    [self.TelField setValue:[UIFont boldSystemFontOfSize:18] forKeyPath:@"_placeholderLabel.font"];
    [self.passwordField setValue:[UIColor whiteColor] forKeyPath:@"_placeholderLabel.textColor"];
    [self.passwordField setValue:[UIFont boldSystemFontOfSize:18] forKeyPath:@"_placeholderLabel.font"];
}
...
#warning 这块的问题是，每次启动App的时候都会闪一次登录页面❤️❤️❤️
...
- (void)checkFieldValid{
    ...
    if (self.passwordField.text.length<6) {
        [[Toast shareToast] makeText:@"密码格式错误，应为6-20位字符" aDuration:1];
        return;
    }
    [self getLogin];
}
...
- (void)getLogin{
    ...
    [vm loginWithPhone:self.TelField.text WithPassWord:self.passwordField.text callback:^(UserInfoModel *userInfo) {
        if (userInfo) {
            ...
            [usvm getBindCarListWithUserId:[userInfo.driverId intValue] callback:^(CarInfoModel *info) {
...
//            [vm selectOrderWithType:0 WithDriverid:[userInfo.driverId intValue] callback:^(NSMutableArray *arrInfo) {
//
//                if (arrInfo.count > 0 || userInfo.userType != 3) {
////                    [ws.navigationController pushViewController:[ZCMyOrderViewController new] animated:NO];
//                    SelfDefineTabBarViewController * controller = [[UIStoryboard storyboardWithName:@"Business" bundle:nil]instantiateViewControllerWithIdentifier:@"SelfDefineTabBarViewController"];
//                    MLNavigationController * navi = [[MLNavigationController alloc]initWithRootViewController:controller];
//                    AppDelegate * app = (AppDelegate *)[UIApplication sharedApplication].delegate;
...
//    RegisterViewController * vC = [[RegisterViewController alloc]init];
    RegisterViewController * controller = [[UIStoryboard storyboardWithName:@"Person" bundle:nil] instantiateViewControllerWithIdentifier:@"RegisterViewController"];
    [self.navigationController pushViewController:controller animated:YES] ;
    
}
//忘记密码
- (IBAction)pressForgetBtn:(id)sender {
//    ResetPasswordViewController * vC = [[ResetPasswordViewController alloc]init];
    ResetPasswordViewController * controller = [[UIStoryboard storyboardWithName:@"Person" bundle:nil] instantiateViewControllerWithIdentifier:@"ResetPasswordViewController"];
    [self.navigationController pushViewController:controller animated:YES] ;
}
