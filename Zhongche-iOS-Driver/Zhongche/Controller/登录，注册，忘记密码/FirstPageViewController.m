...
- (void)bindView{
    ...
    self.ivCartoon.frame = CGRectMake(SCREEN_W/2 -75, 60, 150, 140);
    [self.view addSubview:self.ivCartoon];
    
    self.firstLabel.frame = CGRectMake(0, self.ivCartoon.bottom + 20, SCREEN_W, 30);
    [self.view addSubview:self.firstLabel];
    
    self.lbText.frame = CGRectMake(0, self.firstLabel.bottom+60, SCREEN_W, 30);
    ...
    self.btnPerfect.frame = CGRectMake(20, SCREEN_H - 180, SCREEN_W - 40, 50);
...
- (UIImageView *)ivCartoon {
    if (!_ivCartoon) {
        ...
        imageView.image = [UIImage imageNamed:@"zhuce_pic_1"];
...
- (UIButton *)btnPerfect {
    if (!_btnPerfect) {
        ...
        [button.layer setMasksToBounds:YES];
        [button.layer setCornerRadius:25.0];//设置矩形四个圆角半径
        _btnPerfect = button;
    }
    return _btnPerfect;
}
